...
description: "Migrated source command `evaluate`"
---
...
`.Codex/sprints/{slug}-eval.md` 평가 결과 파일을 생성한다.
...
- `.Codex/sprints/{slug}-contract.md` 가 존재해야 한다
...
### 1단계 — 계약 파일 읽기
- `.Codex/sprints/{slug}-contract.md` 를 읽는다
...
### 5단계 — 결과 파일 작성
아래 형식으로 `.Codex/sprints/{slug}-eval.md` 를 작성한다.
...
### 6단계 (--next-sprint 옵션)
실패한 항목과 발견된 버그를 기반으로 `.Codex/sprints/{slug}-v2-contract.md` 초안을 생성한다.
- 이번 스프린트에서 실패한 완료 기준을 최우선으로 반영
- 새로 발견된 버그 수정을 구현 범위에 추가
- 상태는 `협의중` 으로 설정

### 7단계 (--update-tasks 옵션)
`.Codex/tasks/` 디렉터리의 기존 태스크 파일들을 읽고, 평가 결과를 반영해 우선순위를 재정렬한다.
...
- 스크린샷은 `.Codex/assets/screenshots/sprint-{slug}-{번호}.png` 에 저장한다
