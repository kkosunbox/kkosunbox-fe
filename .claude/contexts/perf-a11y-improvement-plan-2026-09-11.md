...
# T5. Griun PolFairness 폰트 서브셋 (저우선순위 보류)

> **상태: 보류 (2026-09-17 사용자 결정).** 현재 전체 WOFF2 815KB 전송은 감당 가능한 수준이며,
> 서브셋 도입은 글리프 누락 시 조용한 Pretendard 폴백, 전역 줄바꿈 변화, FOUT 재발 가능성 때문에
> 절감 효과에 비해 검증 비용과 리스크가 크다. 지금은 구현하지 않는다.
>
> **재개 조건:** 실제 서비스 체감 속도가 느려지거나, Lighthouse/네트워크 실측에서 폰트 전송이
> 주요 병목으로 다시 확인되거나, 페이지·문구 증가로 815KB 전체 다운로드가 더 이상 수용하기
> 어렵다고 판단될 때 착수한다. 재개 시에는 원본 cmap과 전체 서브셋 cmap 합집합의 완전 일치,
> 전체 관련 시각회귀 무변화, 페이지별 폰트 전송량 감소를 필수 채택 게이트로 삼는다.
