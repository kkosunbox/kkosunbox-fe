...
## 0. 백엔드 실제 계산 규칙 (권위 — 프론트는 **계산하지 않고 받아 쓴다**)

> **2026-08-31 갱신 — 프론트는 더 이상 이 식을 구현하지 않는다.**
> 아래 표는 서버 동작을 이해하기 위한 참고이지 프론트가 재현할 대상이 아니다.
> 표시 단가는 서버가 내려주는 값을 그대로 쓴다:
> - 목록·미리보기 → `GET /v1/subscriptions/plans?referralCode=` 의 `referralDiscountedPrice`
> - 체크아웃 → `POST /v1/subscriptions/price` · `POST /v1/products/{id}/price`
>
> 프론트가 이 식을 따로 구현했다가 **100원 단위 내림을 빠뜨려** 미리보기가 실제
> 청구액보다 35~85원 싸게 표시되고 있었다(dev 실측: 베이직 서버 15,300 / 프론트 15,215).
> 아래 표의 초대코드 행도 그 시절엔 내림이 빠진 채 기록돼 있었다 — 문서와 코드가 같이
> 틀려 있어서 리뷰로 걸리지 않았다.
...
| **초대코드** | **`floor(floor(단가 × 레퍼럴 할인율) / 100) × 100`** — 100원 단위 내림 |
| 기준가 | 쿠폰·초대코드 **둘 다 원가 단가(`plan.monthlyPrice`) 기준** |
| 적용 수량 | **둘 다 1개분만.** 수량 > 1이면 나머지는 정가 |
| 하한 | 0원 |

```
단가 17,900 / 초대 15% (2026-08-31 dev 실측)
  floor(17,900 × 0.15)       = 2,685
  floor(2,685 / 100) × 100   = 2,600   ← 100원 단위 내림
  referralDiscountedPrice    = 15,300
```
