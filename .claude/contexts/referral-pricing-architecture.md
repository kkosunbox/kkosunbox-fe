...
### 실화면 실측 — **적격 경로까지 통과** (2026-08-12)

백엔드가 일시적으로 구독 목록을 비워 내려주는 동안 계정이 "이력 없음 = 적격"으로 잡혀
**할인 경로를 그대로 실측할 수 있었다.** 프리미엄 정가 33,900 / 플랜가 28,900 / 초대 15%.
...
부적격 계정(구독 이력 있음)에서도 네 화면 모두 28,900원으로 일치함을 앞서 확인했다.
...
적격 판정은 `/v1/subscriptions`가 **빈 배열을 200으로** 돌려주면 "이력 없음"과 구분할 수 없다.
오늘 실제로 그 상태가 발생했고(위 실측이 그 덕에 가능했다), 그동안 이력이 있는 사용자에게도
첫 구독 할인가가 노출됐다. `null`(에러) 분기로 막을 수 있는 건 **에러일 때뿐**이다.
서버가 조용히 빈 값을 주는 상황은 백엔드 신뢰성 문제로 남는다.
