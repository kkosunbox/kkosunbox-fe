...
### 3-6. 팝업(480px)보다 Toss 카드 입력창이 넓어 좌우가 잘린다 ⚠️ 대응 필요

2026-07-27 확인. 우리 결제 팝업은 `width=480,height=700`인데, PC에서 Toss는 기본값
`windowTarget: "iframe"` 으로 **고정 폭 데스크탑 레이아웃**을 띄운다. 480px 창 안에서
카드번호 입력칸 오른쪽과 안내 문구 좌우가 잘린다(입력 자체는 가능하나 신뢰도가 떨어짐).

팝업을 여는 곳은 4군데 — `widgets/order/.../usePaymentState.ts`, `widgets/mypage/ui/PaymentCard.tsx`,
`PaymentManagementSection.tsx`, `SubscriptionManagementSection.tsx` (모두 480×700 하드코딩).
