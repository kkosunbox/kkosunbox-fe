...
**부수 효과 1 (수정 완료, 2026-07-27):** 우리 자체 화면(방식 선택·기존 카드·카드 등록 트리거)은
`app/payment/layout.tsx`의 `max-w-[400px]` 중앙 정렬 wrapper를 그대로 쓰다 보니, 650px 창에서는
좌우 여백이 과하게 커 보이는 문제가 실제로 있었다. 기존 480:400 비율을 유지해 `max-w-[540px]`로
조정 완료. success/fail 페이지(`max-w-xl`=576px)는 이 540px가 더 좁아 그대로 적용되며, 400 대비
살짝 넓어진다(부작용 아님, 자연스러운 결과).
