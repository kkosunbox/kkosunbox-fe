...
사용자가 별도 세션에서 `pnpm dev`를 상시 실행하며 직접 확인하는 경우가 많다. 검증 목적으로 아래 작업을 임의로 수행하지 않는다.

* `pnpm build` 실행
* `.next` 등 빌드 캐시 삭제
* 실행 중인 dev 서버 프로세스 종료·재시작

기본 검증은 `tsc`(타입 검사)와 `pnpm lint`만 사용한다. 위 작업이 꼭 필요한 경우 사용자에게 먼저 확인한다.
