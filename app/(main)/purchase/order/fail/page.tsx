...
export default async function PurchaseOrderFailPage({
  ...
  const { code, message, orderId, tier } = await searchParams;
    ...
    <div className="pt-[var(--header-offset)]">
      <div className="mx-auto flex max-w-xl flex-col gap-4 px-6 py-16">
        ...
        <div className="mt-2 flex items-center gap-4">
          {tier ? (
            <Link
              href={`/purchase/order?tier=${tier}`}
              className="text-body-14-sb text-[var(--color-primary)] hover:underline"
            >
              ← 다시 시도하기
            </Link>
          ) : null}
          <Link href="/purchase" className="text-body-14-sb text-[var(--color-text-secondary)] hover:underline">
            구매하기로 돌아가기
          </Link>
        </div>
      </div>
