/**
 * 초대코드(레퍼럴) 할인 계산 — `/subscribe` 플랜 미리보기(`PlanPicker`) 전용.
 *
 * `/order`·`/purchase/order` 체크아웃은 더 이상 이 계산식을 쓰지 않는다 —
 * `POST /v1/subscriptions/price`·`POST /v1/products/{id}/price`가 쿠폰·초대코드까지
 * 반영한 최종 결제 예정 금액을 직접 계산해 내려주므로, 그 응답을 그대로 표시한다
 * (`widgets/order/ui/order-section/hooks/useSubscriptionPriceQuote.ts`).
 *
 * 이 모듈은 아직 그 quote API를 호출할 수 없는 화면 — 로그인 여부와 무관하게 접근 가능한
 * `/subscribe` 플랜 목록·초대 페이지 미리보기 — 에서만 쓰인다(quote 엔드포인트는 인증 필요).
 *
